/**
 * Five screens, and a hash for a router.
 *
 * `#/` is the catalog, `#/approvals` is the queue a person works, `#/evals` is the
 * suite and its publish gate, `#/metrics` is the operations dashboard, and
 * `#/runs/<id>` is one run's trace. A router library would be five routes' worth of
 * dependency; a hash keeps every screen's URL reloadable, shareable and
...
function parseRoute(hash: string): Route {
  ...
  if (/^#\/metrics\b/.test(hash)) return { name: 'metrics' }
  return { name: 'agents' }
}
...
export default function App() {
  ...
  return (
    <Shell active={route.name}>
      ...
      {route.name === 'metrics' && <MetricsScreen onOpenRun={openRun} />}
