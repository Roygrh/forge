workspace "Forge" "A governed agent factory" {
    model {
        ...
        intake = softwareSystem "Email Intake (simulated)" "Invoice PDFs arrive here" "External"

        forge = softwareSystem "Forge" "Governed agent platform" {
            spa = container "Web App" "Catalog, approval queue, trace viewer" "React 18 + Vite + TS"
            api = container "Backend API" "REST API + agent platform" "Python 3.12 + FastAPI" {
                ...
                knowledge = component "Knowledge Retrieval" "Hybrid semantic and lexical retrieval; authority hierarchy; returns citations" "Python + pgvector"
                ...
                breaker = component "Circuit Breaker" "Trips on error or cost thresholds per window; suspends the agent" "Python"
            }
        ...
        spa -> dnaregistry "Create/edit/version DNA (catalog)"
        spa -> approvals "List queue; approve/reject"
        spa -> evalrunner "Run suite / publish"
        spa -> eventwriter "Reads run traces"
        ...
        runtimeloop -> knowledge "Retrieves governed rules/policies"
        ...
        runtimeloop -> breaker "Checks agent health before acting"
